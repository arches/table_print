...
describe ConfigResolver do
  ...
  describe "#default_display_methods" do
    it "returns attribute getters" do
      ...
      columns = ConfigResolver.new(config, Sandbox::Hat.new).columns
    ...
    it "ignores methods defined in a superclass" do
      ...
      columns = ConfigResolver.new(config, Sandbox::Hat::Bowler.new).columns
    ...
    it "uses the members method when passed a Struct" do
      ...
      columns = ConfigResolver.new(config, obj).columns
  ...
  describe "#columns" do
    ...
    it "pulls the column names off the data object" do
      ...
      columns = ConfigResolver.new(config, Sandbox::Post.new).columns
    ...
    it 'pull the column names off of the array of Structs' do
      ...
      columns = ConfigResolver.new(config, data).columns
    ...
    it "pulls out excepted columns" do
      ...
      columns = ConfigResolver.new(config, Sandbox::Post.new, :except => :title).columns
    ...
    it "adds included columns" do
      ...
      columns = ConfigResolver.new(config, Sandbox::Post.new, :include => :author).columns
  ...
  describe ":only" do
    context "with a symbol" do
      it "returns a column named foo" do
        columns = ConfigResolver.new(config, Struct.new(:title), :foo).columns
    ...
    context "with an array of symbols and strings" do
      it "returns columns named foo and bar" do
        columns = ConfigResolver.new(config, Struct.new(:title), :foo, 'bar').columns
  ...
  describe ":include" do
    context "with a symbol" do
      it "adds foo to the list of methods" do
        columns = ConfigResolver.new(config, Object, [:title], :include => :foo).columns
    ...
    context "with an array" do
      it "adds foo and bar to the list of methods" do
        columns = ConfigResolver.new(config, Object, [:title], :include => [:foo, :bar]).columns
    ...
    context "with options" do
      it "adds foo to the list of methods and remembers its options" do
        columns = ConfigResolver.new(config, Object, [:title], :include => {:foo => {:fixed_width => 10}}).columns
  ...
  describe ":except" do
    context "with a symbol" do
      it "removes foo from the list of methods" do
        columns = ConfigResolver.new(config, Object, [:title, :foo], :except => :foo).columns
        expect(columns.collect(&:name)).to eq(%w{title})
      end
    end
    context "with an array" do
      it "removes foo and bar from the list of methods" do
        columns = ConfigResolver.new(config, Object, [:title, :foo, :bar], :except => [:foo, 'bar']).columns
  ...
  describe "lambdas" do
    it "uses the key as the name and the lambda as the display method" do
      lam = lambda {}
      columns = ConfigResolver.new(config, Struct.new(:title), :foo => {:display_method => lam}).columns
    ...
    context "without the display_method keyword" do
      it "uses the key as the name and the lambda as the display method" do
        lam = lambda {}
        columns = ConfigResolver.new(config, Struct.new(:title), :foo => lam).columns
  ...
  describe "#usable_column_names" do
    it "returns default columns" do
      columns = ConfigResolver.new(config, Object, [:title]).columns

      expect(columns.collect(&:name)).to eq(%w{title})
    end

    it "returns specified columns instead of default columns" do
      columns = ConfigResolver.new(config, Struct.new(:title), [:author]).columns

      expect(columns.collect(&:name)).to eq(%w{author})
    end

    it "applies includes on top of default columns" do
      columns = ConfigResolver.new(config, Object, [:title], [:include => :author]).columns

      expect(columns.collect(&:name)).to eq(%w{title author})
    end

    it "applies includes on top of specified columns" do
      columns = ConfigResolver.new(config, Struct.new(:title), [:author, {:include => :pub_date}]).columns

      expect(columns.collect(&:name)).to eq(%w{author pub_date})
    end

    it "doesn't double up if a default column is re-specified" do
      columns = ConfigResolver.new(config, Object, [:title, :author], [:include => :author]).columns

      expect(columns.collect(&:name)).to eq(%w{title author})
    end

    it "applies excepts on top of default columns" do
      columns = ConfigResolver.new(config, Object, [:title, :author], [:except => :author]).columns

      expect(columns.collect(&:name)).to eq(%w{title})
    end

    it "doesn't double up on intermediary objects" do
      columns = ConfigResolver.new(config, Object, [:title, :comment], [:include => ["comment.body", "comment.author"]]).columns

      expect(columns.collect(&:name)).to eq(%w{title comment.body comment.author})
    end

    it "applies excepts on top of specified columns" do
      columns = ConfigResolver.new(config, Struct.new(:title, :author), [:pub_date, :length, {:except => :length}]).columns

      expect(columns.collect(&:name)).to eq(%w{pub_date})
    end

    it "applies both includes and excepts on top of specified columns" do
      columns = ConfigResolver.new(config, Struct.new(:title, :author), [:pub_date, :length, {:except => :length, :include => :foobar}]).columns
  ...
  describe "column options" do
    context "display_method" do
      it "sets the display method on the column" do
        columns = ConfigResolver.new(config, Object, [:title], :title => {:display_method => :boofar}).columns
    ...
    context "width" do
      it "sets the default width" do
        columns = ConfigResolver.new(config, Object, [:title], :title => {:fixed_width => 100}).columns
    ...
    context "formatters" do
      it "adds the formatters to the column" do
        f1 = {}
        f2 = {}
        columns = ConfigResolver.new(config, Object, [:title], :title => {:formatters => [f1, f2]}).columns
    ...
    context "display_name" do
      it "sets the display name on the column" do
        columns = ConfigResolver.new(config, Object, [], :title => {:display_name => "Ti Tle"}).columns
