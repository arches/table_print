...
<tt>COL.as(LABEL)</tt>::       use +LABEL+ for the +COL+ header
<tt>COL.from(SRC)</tt>::       use +SRC+ for the data for +COL+
<tt>COL.width(W)</tt>::        set the +COL+ width to +W+
<tt>COL.right(W=nil)</tt>::    set the +COL.align+ to +:right+, in width +W+ if given
<tt>COL.left(W=nil)</tt>::     set the +COL.align+ to +:left+, in width +W+ if given
<tt>COL.center(W=nil)</tt>::   set the +COL.align+ to +:center+, in width +W+ if given
