...
+COLUMN.from(SRC)+::       - use +SRC+ for the data for +COLUMN+
+COLUMN.width(W)+::        - set the +COLUMN+ width to +W+
+COLUMN.right(W=nil)+::    - set the +COLUMN.align+ to +:right+, in width +W+ if given
+COLUMN.left(W=nil)+::     - set the +COLUMN.align+ to +:left+, in width +W+ if given
+COLUMN.center(W=nil)+::   - set the +COLUMN.align+ to +:center+, in width +W+ if given
+COLUMN.formatter(FMT)+::  - set +FMT+ as a formatter for +COLUMN+
+COLUMN.as_num(W=nil)+::   - set +NumFormatter+ as the formatter, with width +W+
+COLUMN.as_money(W=nil)+:: - set +MoneyFormatter+ as the formatter, with width +W+
